...
使用 int32 数据类型避免 bfloat16 精度问题，精确验证 dispatch 正确性。
编码方式：每个 token 所有 hidden 维度填充 rank * 1000000 + token_id，
dispatch 后精确解码 (src_rank, token_id)，零精度损失。
...
def build_encoded_data_int32(num_tokens, hidden, rank, device):
    """
    int32 编码：value = rank * 1000000 + token_id。
    int32 范围 [-2^31, 2^31-1]，支持最多 2000 ranks × 1000000 tokens。
    """
...
def verify_dispatch_int32(recv_x, rank_prefix_matrix, rank, num_ranks,
                          ...
                          verbose=False):
    """
    逐 token 精确验证 dispatch 结果 (int32, 零精度损失)。
    """
    errors = []
    detailed_prints = 0  # 限制详细诊断打印数量
    ...
    for src_rank in range(num_ranks):
        ...
        for idx in range(check_start, check_end):
            ...
            if not (row == val).all():
                ...
                # === 详细诊断打印：逐 int4 (4个int32) 对比 actual vs expected ===
                if detailed_prints < 3:
                    detailed_prints += 1
                    # int4 = 16 bytes = 4 x int32，UNROLLED_WARP_COPY 以 int4 为单位拷贝
                    ...
                    # 打印前 10 个错误的 int4 块（位置 + actual vs expected）
...
def main():
    ...
    if args.profile and rank == 0:
        ...
        config_prof = deep_ep.Config(args.num_sms, 256, 512)
        torch.xpu.synchronize()
