#!/usr/bin/env python
"""XPU notify_dispatch 压力测试

模式:
  verify:   功能正确性验证（多场景 × Python 参考实现对比）
  perf:     e2e 性能测试（Python loop + host 计时）
  profile:  torch.profiler 抓 JSON trace
...
def get_channel_task_range(num_tokens, num_channels, channel_id):
    """与 kernel 中 get_channel_task_range 逻辑一致"""
...
def suppress_cpp_stdout():
    """重定向 fd 1 到 /dev/null，抑制 C++ std::cout 输出"""
    sys.stdout.flush()
...
def reference_notify_dispatch(num_tokens_per_rank_cpu, num_tokens_per_expert_cpu,
    ...
    """
    Python 参考实现。通过 MPI allgather 收集所有 rank 数据并计算期望结果。

    返回: (moe_recv_count, expert_counts, rank_prefix_col, channel_prefix)
    """
    ...
    # 3. rank_prefix_matrix 列 rank 的前缀和
    rank_prefix_col = []
    prefix = 0
...
def generate_topk(scenario, num_tokens, num_topk, num_experts, num_ranks, rank, seed):
    """生成 topk_idx [num_tokens, num_topk] int64"""
    epr = num_experts // num_ranks
...
def verify_one(name, topk_idx_cpu, num_experts, num_channels, expert_alignment,
    ...
    """运行一个测试用例，返回错误列表（空 = 通过）"""
    import deep_ep

    actual_tokens = topk_idx_cpu.shape[0]

    # 通过 get_dispatch_layout 生成输入
...
def run_tests(args):
    ...
    if rank == 0:
        print(f"\n{'='*60}")
        print(f"  notify_dispatch 压力测试 [{mode}]")
...
def main():
    parser = argparse.ArgumentParser(
        description='XPU notify_dispatch 压力测试')
