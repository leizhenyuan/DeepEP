...
hybrid_dispatch_impl(
    ...
    //     workspace 起始地址
    // │
    // ├─ [16 bytes]           NVLink barrier counter + signal (phase 0/1)
    // │
    // ├─ [(1024+2048)*8 bytes] Notify reduction workspace (int64 × max_ranks+max_experts)
    // │                         各 SM 在这里做 GPU-side reduce
    // │
    // ├─ [1024*8*2 bytes]     Scale-up rank/expert count (send + recv, int64)
    // │                         scale-up 方向的 token 计数
    // │
    // ├─ [2048*8*2 bytes]     (包含在上面，expert 部分)
    // │
    // ├─ [1024*4 bytes]       Scale-up atomic sender counter (int × max_ranks)
    // │                         forward warp 分配 slot 用的原子计数器
    // │
    // ├─ [1024*4*2 bytes]     Scale-out rank/expert count (send + recv, int × max_ranks+max_experts)
    // │                         scale-out 方向的 token 计数
    // │
    // ├─ [1024*1024*8 bytes]  Scaleout channel signaled_tail  ← channel 相关
    // │                         int64[kNumMaxRanks × kNumMaxChannels]
    // │
    // ├─ [1024*1024*4 bytes]  Channel scaleup tail            ← channel 相关
    // │                         int[kNumMaxRanks × kNumMaxChannels]
    // │
    // ├─ [4*8 bytes]          PP send/recv count
    // │
    // └─ [(32+1)*1024*4 bytes] AGRS signals
    ...
    if (warp_idx < kNumNotifyWarps) {
        ...
        // 高32位用来记录已经到达的 SM 数量，低32位用来记录计数值
        #pragma unroll
