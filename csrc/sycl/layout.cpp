...
void get_dispatch_layout(const topk_idx_t* topk_idx,
    ...
    int num_expert_blocks = (num_experts + kNumExpertsPerSM - 1) / kNumExpertsPerSM;
    int num_rank_blocks = (num_ranks + kNumRanksPerSM - 1) / kNumRanksPerSM;
    int num_sms = num_expert_blocks + num_rank_blocks;

    // Static assertions
    static_assert(kNumRanksPerSM % NUM_MAX_NVL_PEERS == 0, "Invalid number of ranks per SM");
    ...
    sycl::nd_range<1> nd_range(
        sycl::range<1>(num_sms * kNumThreads),  // global size
    ...
    q.submit([&](sycl::handler& h) {
        ...
        auto local_mem_expert = sycl::local_accessor<int, 2>(
            sycl::range<2>(kNumThreads, kNumExpertsPerSM), h);
        auto local_mem_rank = sycl::local_accessor<int, 2>(
            sycl::range<2>(kNumThreads, kNumRanksPerSM), h);
        auto local_mem_rdma = sycl::local_accessor<int, 2>(
            sycl::range<2>(kNumThreads, kNumRanksPerSM / NUM_MAX_NVL_PEERS), h);
        ...
        h.parallel_for(nd_range, [=](sycl::nd_item<1> item) {
            ...
            int expert_end_idx = sycl::min(expert_begin_idx + kNumExpertsPerSM, num_experts);
            ...
            int rank_begin_idx = (sm_id - num_expert_blocks) * kNumRanksPerSM;
            int rank_end_idx = sycl::min(rank_begin_idx + kNumRanksPerSM, num_ranks);
            int rdma_rank_begin_idx = rank_begin_idx / NUM_MAX_NVL_PEERS;
            int rdma_rank_end_idx = rank_end_idx / NUM_MAX_NVL_PEERS;
