...
SYCL_EXTERNAL inline void barrier_block_cas(int** barrier_signal_ptrs, int rank, 
    ...
    if (thread_id < kNumRanks) {
        *debug_stream << "[barrier_block_cas][Rank " << rank << "][Thread " << thread_id 
                     << "] sg_id=" << sg_id << ", lane_id=" << lane_id << sycl::endl;
    }
    ...
    if (!all_done) {
        if (thread_id == 0 && debug_stream) {
            *debug_stream << "[barrier_block_cas][Rank " << rank << "] BARRIER FAILED! Not all threads done." << sycl::endl;
        }
        // 打印失败的线程
        if (!self_done && debug_stream) {
            *debug_stream << "[barrier_block_cas][Rank " << rank << "][sg " << sg_id 
                         << "] TIMEOUT: sub_spins=" << sub_spins << sycl::endl;
        }
        item.barrier(sycl::access::fence_space::local_space);
        return;
    ...
    if (thread_id == 0 && debug_stream) {
        *debug_stream << "[barrier_block_cas][Rank " << rank << "] barrier_block_cas EXIT: SUCCESS" << sycl::endl;
    }
}
